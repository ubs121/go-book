...
// GraphAdjSet API, undirected
type GraphAdjSet struct {
...
func NewGraph(v int) *GraphAdjSet {
	g := &GraphAdjSet{}
	g.V = v
	g.E = 0
...
func (g *GraphAdjSet) AddEdge(v, u int) {
...
func (g *GraphAdjSet) Degree(v int) int {
	return len(g.Adj[v])
}
...
func NewBFS(g *GraphAdjSet, src int) *BFS {
	bfs := &BFS{}
...
func (bfs *BFS) run(g *GraphAdjSet, s int) {
	var q []int // queue
...
func NewDFS(g *GraphAdjSet, src int) *DFS {
	dfs := &DFS{}
...
func (dfs *DFS) run(g *GraphAdjSet, u int) {
	dfs.count++
	dfs.marked[u] = true
